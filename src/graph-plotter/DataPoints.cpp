...
vector<PointPair> DataPoints::getAsPointPairs() const
{
	...
	auto first_point_in_pair = datapoints_.cbegin();  // Returns a const_iterator, no non-member versions in C++11
	for (; first_point_in_pair != datapoints_.end()-1; first_point_in_pair++)
	{
...
vector<float> DataPoints::extractYValues() const
{
	...
	for (const auto& point : datapoints_)	// store all y values
	{
...
float DataPoints::getMaxX() const
{
	...
	auto last_point = datapoints_.back();
	return last_point.x;
}
...
float DataPoints::getMinX() const
{
	...
	auto first_point = datapoints_.front();
	return first_point.x;
}

void DataPoints::transformToDisplayCoordinateSystem(int display_width, int display_height)
{
	if (transformed_) // it does not make sense to transform points more than once
		return;
	...
	datapoints_ = transformed_points;
	transformed_ = true;
}
